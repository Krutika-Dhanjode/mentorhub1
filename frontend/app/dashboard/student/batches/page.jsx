...
export default function StudentBatchesPage() {
    ...
    useEffect(() => {
        const fetchBatchData = async () => {
            ...
            const peerIds = Array.from(new Set((peerAssignments || []).map((entry) => entry.student_id).filter(Boolean)));
            ...
            const peersById = new Map((peerUsers || []).map((peer) => [peer.id, peer]));
            const formattedAssignments = (batchData || []).map((batch) => ({
                batchId: batch.id,
    ...
    const totalClassmates = useMemo(() => assignments.reduce((sum, batch) => sum + batch.classmates.length, 0), [assignments]);
    ...
    return (<div className="space-y-6">
      ...
      {assignments.length === 0 ? (<Card className="border-border p-6">
          <p className="text-sm text-muted-foreground">
            You have not been added to any batch yet. Ask your mentor to assign you to a batch.
          </p>
        </Card>) : (<div className="space-y-6">
          {assignments.map((assignment) => (<Card key={assignment.batchId} className="border-border p-6">

...
export default function MentorGuidancePage() {
    ...
    const fetchThreads = async () => {
        ...
        const queriedStudentIds = Array.from(new Set((guidanceData || [])
            .filter((entry) => entry.sender_role === "student")
        ...
        const assignedStudentIds = Array.from(new Set((assignmentData || []).map((assignment) => assignment.student_id).filter(Boolean)));
        ...
        const nextThreads = (studentData || [])
            ...
            const latestMentorReply = [...studentMessages]
                .filter((entry) => entry.sender_role === "mentor")
                .sort((left, right) => new Date(right.created_at).getTime() - new Date(left.created_at).getTime())[0];
            const unansweredCount = studentMessages.filter((entry) => {
                if (entry.sender_role !== "student")
                    return false;
    ...
    const handleReply = async () => {
        if (!user || !selectedStudentId || (!draft.trim() && !selectedFile))
            return;
        setSending(true);
        let attachmentUrl = "";
        let attachmentName = "";
        ...
        try {
            ...
            const emailResult = await sendNotificationEmail(
                ...
                "guidance", 
                attachmentUrl
                    ? `${messageText}\nReference: ${attachmentName || "Attachment"}\n${attachmentUrl}`
                    : messageText,
                mentorName
            );
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <Card className="p-4">
          ...
          {threads.length === 0 ? (<p className="text-sm text-muted-foreground">
              No student queries have been posted yet.
            </p>) : (<div className="space-y-3">
              {threads.map((thread) => (<button key={thread.id} type="button" onClick={() => setSelectedStudentId(thread.id)} className={`w-full rounded-lg border p-3 text-left transition-colors ${selectedStudentId === thread.id
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {thread.unansweredCount > 0 && (<Badge>{thread.unansweredCount}</Badge>)}
                  </div>
                </button>))}
            </div>)}
        </Card>
        ...
        <Card className="flex min-h-[560px] flex-col p-4">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto py-4">
            {messages.length === 0 ? (<div className="flex h-full flex-col items-center justify-center text-center text-muted-foreground">
              ...
              </div>) : (messages.map((message) => (<div key={message.id} className={`max-w-[80%] rounded-2xl px-4 py-3 ${message.senderRole === "mentor"
                ...
                : "bg-muted text-foreground"}`}>
                  ...
                  <p className={`mt-2 text-[11px] ${message.senderRole === "mentor"
          ...
          <div className="border-t pt-4">
            <Textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Write your guidance reply..." className="min-h-28"/>
            ...
            <div className="mt-3 flex justify-end">
              <Button onClick={handleReply} disabled={sending || !selectedStudentId || (!draft.trim() && !selectedFile)}>

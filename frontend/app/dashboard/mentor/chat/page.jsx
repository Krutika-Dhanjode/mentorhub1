...
export default function MentorBatchChatIndexPage() {
      ...
      {batches.length === 0 ? (
        <Card className="border-border p-8 text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            Create a batch from the Students page first, then come back to chat with them.
          </p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {batches.map((batch) => (
            <Card
              ...
              onClick={() => router.push(`/dashboard/mentor/batches/${batch.id}/chat`)}
            >

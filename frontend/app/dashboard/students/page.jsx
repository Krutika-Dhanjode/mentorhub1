...
export default function StudentsPage() {
    ...
    const [expandedMentors, setExpandedMentors] = useState(['1', '2', '3']);
    const toggleMentor = (mentorId) => {
        setExpandedMentors((prev) => prev.includes(mentorId)
    ...
    const filteredMentors = mentorsWithStudents.map((mentor) => ({
        ...mentor,
        students: mentor.students.filter((s) => s.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const excellentStudents = mentorsWithStudents.reduce((acc, m) => acc + m.students.filter((s) => s.status === 'Excellent').length, 0);
    const atRiskStudents = mentorsWithStudents.reduce((acc, m) => acc + m.students.filter((s) => s.status === 'At Risk').length, 0);
    return (<div className="space-y-6">
      ...
      <div className="flex gap-3">
        <div className="flex-1 relative">
          ...
          <Input placeholder="Search students by name, PRN, or email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 bg-input border-border"/>
        </div>
      </div>

      {/* Students by Mentor */}
      <div className="space-y-4">
        {filteredMentors.map((mentor) => (<Card key={mentor.id} className="border-border overflow-hidden">
            <Collapsible open={expandedMentors.includes(mentor.id)} onOpenChange={() => toggleMentor(mentor.id)}>
              <CollapsibleTrigger className="w-full px-6 py-4 flex items-center gap-4 hover:bg-secondary/30 transition-colors">
                {expandedMentors.includes(mentor.id) ? (<ChevronDown className="w-5 h-5 text-muted-foreground"/>) : (<ChevronRight className="w-5 h-5 text-muted-foreground"/>)}
              ...
              <CollapsibleContent>
                <div className="px-6 pb-4">
                  <Table>
                    ...
                    <TableBody>
                      {mentor.students.map((student) => (<TableRow key={student.id} className="border-border hover:bg-secondary/30 transition-colors">

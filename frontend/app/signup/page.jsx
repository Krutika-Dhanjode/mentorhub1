...
export default function SignupPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (profileSaveError) {
            alert('Signup succeeded, but saving profile failed: ' + profileSaveError.message);
            setIsLoading(false);
            return;
        }
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-background via-secondary to-accent/10 flex items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-lg border-0">
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            const Icon = roleOption.icon;
            return (<button key={roleOption.id} type="button" onClick={() => setRole(roleOption.id)} className={`flex flex-col items-center gap-1 p-3 rounded-lg border-2 transition-all duration-200 ${role === roleOption.id
                    ? 'border-primary bg-primary/10 text-primary'
                    : 'border-border bg-input hover:border-primary/50 text-muted-foreground hover:text-foreground'}`}>
                      <Icon className="w-5 h-5"/>
                      <span className="text-xs font-medium">{roleOption.label}</span>
